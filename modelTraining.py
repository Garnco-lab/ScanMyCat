...
for i, id in enumerate(df_labels["id"]):
    ...
    temp_image = cv2.resize(
        cv2.imread(train_folder + id + ".jpg", cv2.IMREAD_COLOR),
        ((image_size, image_size)),
    )

    image_list = preprocess_input(
        np.expand_dims(
            np.array(temp_image[..., ::-1].astype(np.float32)).copy(), axis=0
        )
    )
...
x_trainingModel, x_testModel, y_trainingModel, y_testModel = train_test_split(
    input_for_model, target_for_model, test_size=0.2, random_state=42
)
...
train_image_generator = train_data_generator.flow(
    x_trainingModel, y_trainingModel, batch_size=batch_size
)

test_data_generator = ImageDataGenerator()

test_generator = test_data_generator.flow(
    x_testModel, y_testModel, batch_size=batch_size
)
