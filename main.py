...
prediction_image_path = 'frolicfunny.jpg'

prediction_image_list = cv2.resize(cv2.imread(prediction_image_path, cv2.IMREAD_COLOR), ((image_size, image_size)))

prediction_image_list = preprocess_input(np.expand_dims(np.array(prediction_image_list[..., ::-1].astype(np.float32)).copy(), axis=0))

# feed the model with the image array for prediction
pred_val = model.predict(np.array(prediction_image_list, dtype="float32"))
# display the image of dog

# display the predicted cat breed
pred_breed = sorted(breed_dictionary)[np.argmax(pred_val)]
